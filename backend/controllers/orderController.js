...
export const createNewOrder = handleAsyncError(async (req, res, next) => {
  if (!req.body || Object.keys(req.body).length === 0) {
    return next(new HandleErroe("Request body is required. Send JSON with Content-Type: application/json", 400));
  }
  ...
  if (!shipingInfo || !orderItem || !paymentInfo) {
    return next(new HandleErroe("shipingInfo, orderItem and paymentInfo are required", 400));
  }
