    ...
    <div className="bg-white dark:bg-slate-800/60 backdrop-blur-sm border border-gray-200 dark:border-slate-700/50 rounded-2xl shadow-2xl px-8 py-10">
      ...
      <div className="flex justify-center mb-6">
        <div className={`w-20 h-20 rounded-full overflow-hidden border-2 shadow-lg transition-all duration-200
          ${isValid('avatar') ? 'border-emerald-400/60 shadow-emerald-400/10' : 'border-amber-500/30 shadow-amber-500/10'}`}>
          <img src={avatarPreview} className="w-full h-full object-cover" alt="Preview" />
        </div>
      </div>

      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Sign Up</h2>
        <p className="mt-2 text-gray-500 dark:text-slate-400 text-sm">Create your account to get started.</p>
      </div>
      ...
      <form onSubmit={registerSubmit} encType="multipart/form-data" className="space-y-5" noValidate>
        ...
        {/* Name */}
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-slate-400 mb-1.5">Username</label>
          <div className="relative">
            <span className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
              <svg className={`w-4 h-4 ${showError('name') ? 'text-red-400' : isValid('name') ? 'text-emerald-400' : 'text-gray-400 dark:text-slate-500'}`} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        ...
        {/* Email */}
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-slate-400 mb-1.5">Email</label>
          <div className="relative">
            <span className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
              <svg className={`w-4 h-4 ${showError('email') ? 'text-red-400' : isValid('email') ? 'text-emerald-400' : 'text-gray-400 dark:text-slate-500'}`} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        ...
        {/* Password */}
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-slate-400 mb-1.5">Password</label>
          <div className="relative">
            <span className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
              <svg className={`w-4 h-4 ${showError('password') ? 'text-red-400' : isValid('password') ? 'text-emerald-400' : 'text-gray-400 dark:text-slate-500'}`} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        ...
        {/* Confirm Password */}
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-slate-400 mb-1.5">Confirm Password</label>
          <div className="relative">
            <span className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
              <svg className={`w-4 h-4 ${showError('confirmPassword') ? 'text-red-400' : isValid('confirmPassword') ? 'text-emerald-400' : 'text-gray-400 dark:text-slate-500'}`} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
              </svg>
            </span>
            <input
        ...
        {/* Avatar Upload */}
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-slate-400 mb-1.5">Avatar</label>
          <label className={`flex items-center justify-center gap-2 w-full py-3 px-4 rounded-xl bg-gray-50 dark:bg-slate-900/70 border border-dashed cursor-pointer transition-all duration-200 text-xs font-semibold uppercase tracking-widest
            ${showError('avatar')
              ? 'border-red-500 text-red-400 hover:border-red-400'
              : isValid('avatar')
                ? 'border-emerald-500/60 text-emerald-400 hover:border-emerald-400'
                : 'border-amber-500/30 text-amber-500/60 hover:border-amber-500/60 hover:text-amber-500'
            }`}>
        ...
        <div className="pt-1">
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 px-6 rounded-xl font-bold text-sm tracking-widest uppercase transition-all duration-200 bg-amber-500 hover:bg-amber-400 dark:hover:bg-amber-300 text-slate-900 shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? (
